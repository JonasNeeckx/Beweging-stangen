...
decay1 = exp(-zeta*2*pi*lambda*(tau - 1));
...
denominator = [1, 2*zdeta*(2*pi*lambda), (2*pi*lambda)^2];
sys = tf(numerator, denominator);

approx_input = (((2*pi)^2*((tau_SR - 1).^3))/factorial(3)) + 1;
...
x_0 = gamma_num_SR(8000) - 1;
lambda_d = lambda * sqrt(1 - zdeta^2);
dx_0 = (gamma_num_SR(8001) - gamma_num_SR(7999))/(2*0.0001);
A_num = sqrt(((x_0*2*pi*lambda_d)^2 + (dx_0 + zeta*2*pi*lambda*x_0)^2)/((2*pi*lambda_d)^2));
A_approx = ((Q)/((2*pi*lambda)^3))*sqrt(1/(1-zeta^2));
...
figure
subplot(2, 1, 1)
plot(tau_SR(8000:22001), gamma_approx + 1, 'LineWidth', 2)
xlabel('tau [-]')
...
subplot(2, 1, 2)
plot(tau_SR(8000:22001), gamma_approx + 1 - gamma_num(8000:22001), 'LineWidth', 2)
xlabel('tau [-]')
